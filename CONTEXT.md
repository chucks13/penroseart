...
The always-running runtime service every consumer — effects, transitions, any system — pulls from — **its own readable surface beside the Data Surface**: BeatManager answers what the music is doing, the synthesizer plays the wall's rhythm, and consumers freely mix reads from both in the same frame. The live pulse keeps a Bar Phase clock turning; given any Waveform, the synthesizer evaluates it against the current Bar Phase and hands back a brightness in `[0..1]` on demand — nullable: with no clock running there is no bar position, so the evaluation reads null and each consumer chooses its own Standalone response. Effects do not own the clock — they hold a Waveform value (drawn at random by Energy, requested by Preset name, or typed inline) and ask for its value *now*. Evaluation is the synthesizer's one primitive; brightness and time seasoning live effect-side. The Waveform Hold is the synthesizer's inbound knob.
_Avoid_: index addressing in any form (a Pool position may change at any time; names are the stable handle); a second evaluation spelling on the provider; reaching the synthesizer through a BeatManager doorway (it is a sibling, not a child); "Wall Variant Lock" (retired name — see **Waveform Hold**).
...
The second concrete use of **Hold**: pin the whole wall to one chosen Waveform so a person can watch exactly that rhythm play and examine how effects react to it — the current effect retargets immediately, and every effect that starts while it is engaged uses the held Waveform. Releasing it returns the wall to **Auto**: each effect acquires its own Waveform again. It holds a Waveform value, never a Pool position. An inspection affordance of the runtime that editor surfaces actuate, not own.
...
**Routine**:
A 16-beat choreography — exactly one Grid — assembled from single-bar Waveforms in four one-bar slots. Each slot is "how this bar gets its Waveform": a random draw at an Energy level (so a sequence like Low, Low, High, Mid dances fresh steps) or a pinned Waveform (so a chosen move always lands its bar); a silent bar is just an all-zero Waveform. Sequences are authored by the effect or rolled at random — fully random, or slot-by-slot within a chosen set of Energy levels. A Routine wraps to the Grid whenever the wire places one, whatever the Grid State says about trust; refreshing its draws — on each new Grid, or on demand — is the holder's choice, offered both as a plain re-roll and as an opt-in per-wrap mode. It composes the same Waveforms the synthesizer already evaluates: its one live read is its envelope at the current Grid position (the Grid's bar picks the slot), null when there is no Grid. The Waveform Hold reaches through it while engaged — every slot plays the held Waveform.
_Avoid_: "Waveform Pattern" (retired name); any Routine length other than one Grid; using "pattern" for one bar's hump sequence; treating a Routine as a new evaluation model; automatic refresh nobody asked for (refresh is the holder's choice); gating the Routine on Grid State (trust is served data for effects to weigh, not a gate the Routine applies).
