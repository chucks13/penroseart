...
**Waveforms**:
The shared acquisition and evaluation surface beside BeatManager: consumers draw or look up Waveform values, evaluate them against BeatManager's Bar Phase, read hit Edges, and compose Routines. Consumers own every acquired value and response; the Waveform Hold is the surface's runtime-owned inspection control.
_Avoid_: index addressing in any form (a Pool position may change at any time; names are the stable handle); a second evaluation spelling on the provider; reaching Waveforms through a BeatManager doorway (it is a sibling, not a child); "Wall Variant Lock" (retired name — see **Waveform Hold**).

**Preset**:
A named, saved Waveform spec — a convenience handle for a `sequence + amplitude + rounding + offset` bundle so it can be referred to by name instead of retyping the notation. Presets are *optional* for any single lookup: Waveforms works with inline and named values. The plain Beat Pulse (`QQQQ` / `8888`) is the canonical default.
...
The hand-vetted collection of Presets that random selection draws from, so a random pick is always musically sensible. It is the **runtime source of truth** for available Presets, persisted as a **hand-editable text file in `StreamingAssets`** — in the spirit of `palettedata.txt`: named entries, plain notation, `//` comments, blank lines ignored, creatable by hand in any text editor. `WaveformPool` owns the shared runtime/editor codec; Waveforms loads the Pool for acquisition, while the Unity editor provides the authoring UI. A file — not a scene-serialized field — keeps runtime acquisition independent of editor code, and a human remains a first-class author alongside the editor. The 7 legacy variants seed the default Pool. Inline specs bypass the Pool entirely.

**Waveform Hold** (released state: **Auto**):
The second concrete use of **Hold**: pin the whole wall to one chosen Waveform so a person can watch exactly that rhythm play and examine how effects react to it. Existing evaluations retarget immediately, and single-Waveform acquisitions while engaged return the held value. Releasing returns to **Auto** by clearing substitution: caller-held values become visible again, and future acquisitions use their requested source. Release never tells an effect or transition to reacquire or replace anything. It holds a Waveform value, never a Pool position. An inspection affordance of the runtime that editor surfaces actuate, not own.
_Avoid_: "Wall Variant Lock" (retired name; "variant" terminology retires with it — accurate in Chuck's original sense, but Waveform is the canonical word); treating the hold as a performance feature; editor surfaces originating or owning the hold state.

**Routine**:
A 16-beat choreography — exactly one Grid — containing four resolved single-bar Waveforms. An effect or transition acquires it explicitly: fully random, slot-by-slot from Energy draws, or with pinned Waveforms and Preset names. The Routine retains only those four resolved values, never the acquisition settings or replacement policy. Its one live read is the current Grid-position envelope (the Grid's bar picks the slot), null when there is no Grid; Grid State remains trust data for the consumer. A consumer wanting different values explicitly acquires another Routine whenever it chooses. The Waveform Hold reaches through evaluation while engaged without rewriting the held Routine.
_Avoid_: "Waveform Pattern" (retired name); any Routine length other than one Grid; using "pattern" for one bar's hump sequence; treating a Routine as a new evaluation model; putting re-roll, refresh, wrap, or lifecycle policy on Routine, Waveforms, or a base hook; gating evaluation on Grid State.
...
**On the Beat** (`OnBeat`):
Landing on the count — on the 1, 2, 3, or 4. The wire reports this per count as four gates; a count's gate is open for the first quarter of the beat interval. The Data Surface serves one contrived convenience, **On Beat**: is the *current* count's gate open right now. Watching specific counts (the 2 and the 4 for the snare) reads the four per-count gates.
...
**Data Surface**:
The read-only face of BeatManager through which every consumer — effects, transitions, Waveforms, any future system — pulls musical data. One surface, organized by musical concept (clock, beat moments, events, energy, levels, …): every datum lives exactly once, in the group its meaning belongs to, raw and contrived values side by side — provenance is the provider's bookkeeping, invisible at the point of use. Nothing arriving over the wire is ever dropped, and every wire fact stays pullable by name. Uniformly nullable — `null` is the only public spelling of "not available"; wire sentinels never cross it. Reads cannot write: data flows one way, from the wire through BeatManager outward. The few inbound knobs (transport feed and tuning; the Waveform Hold lives on Waveforms) sit apart from the Data Surface; they are control, not data.
_Avoid_: a second spelling of any datum (duplication); a separate consumer-facing "raw wire view" (retired — two kinds of data, never two structures); dropping a wire lane because nothing reads it yet; mingling control knobs among data reads; treating the surface as restricting what a consumer may read or combine; merging similar-looking offerings without proof they are duplicates; surfacing transport/wire liveness as an offering (`IsLiveSource`, retired) — connectivity is internal plumbing that debug views mirror.
