...
**Director**:
The decision layer that owns *what* plays on the wall. Its whole job in Synced Mode: keep one track-scoped Cue Sheet per player (rebuilt when that player's Structure Generation changes), Cast the on-air player's sheet to the Switcher when the focus changes, and answer the Switcher's staleness asks once it reports nothing has fired for a while — declining the early ones so the wall holds, and dealing a fresh card by the time the deficit reaches the plan's own maximum Cue Mark spacing. It reads musical truth only from BeatManager and times nothing.
...
_Avoid_: the retired per-Phrase empty-marks sheet (marks now carry assignments); treating it as a queue of pending cues; mutating it for overrides or staleness (masks and staleness deals leave the plan untouched); recording check-offs on the sheet itself (what has already been performed is the Switcher's execution state); caching or keying sheets by track id.
...
**Cast**:
The Director's act of handing the Switcher the Cue Sheet now in force — the cast list for the on-air track, handed over once when the focus changes. A staleness cue is not a Cast: the Switcher already holds the plan, and the Director simply answers with one freshly dealt card, seeded so no two asks are answered alike.
...
**Bag**:
The seeded fairness mechanism sheet building deals from: one shuffled bag per catalog (Effects, Transitions), dealt top-card so the full catalog shows before any repeat, reshuffled when empty. Capability and soft energy scans may dig into the bag; a discard-pile encore is allowed only when an Anchor finds no capable card left.
_Avoid_: weights or scoring (the bag *is* the fairness); per-cast random picks; sharing one bag across sheets or players.
