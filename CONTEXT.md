...
- **Waveforms**: Controller owns one `Waveforms` acquisition surface beside BeatManager and exposes it to Effects and Transitions as `waveforms`. Performers acquire immutable, clock-bound Waveform values; the held value reads its own live `Envelope` or maps it through `Lerp(from, to)`. Legacy index-addressed Variant members remain only for consumers awaiting the final contract cut.
