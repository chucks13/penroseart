...
**Bag**:
The seeded fairness mechanism sheet building deals from: one shuffled bag per catalog (Effects, Transitions), dealt top-card so the full catalog shows before any repeat, reshuffled when empty. The reshuffle keeps the card just dealt off the top of the new permutation, because the seam between two passes is the one place a fair bag would otherwise deal the same card twice running — and an Effect dealt twice running bakes a Transition from a card to itself, which moves nothing. Capability and soft energy scans may dig into the bag; a discard-pile encore is allowed only when an Anchor finds no capable card left, and is the one path that can still repeat a card back to back.
_Avoid_: weights or scoring (the bag *is* the fairness); per-cast random picks; sharing one bag across sheets or players; treating a one-card catalog's unavoidable repeat as a defect.
