...
**Waveforms**:
The shared acquisition and evaluation surface beside BeatManager: consumers draw or look up Waveform values, evaluate them against BeatManager's Bar Phase, read hit Edges, and evaluate Routines. Consumers own every acquired value and response.
_Avoid_: index addressing in any form (a Pool position may change at any time; names are the stable handle); a second evaluation spelling on the provider; reaching Waveforms through a BeatManager doorway (it is a sibling, not a child); runtime waveform substitution or a "Wall Variant Lock" inspection path.
...
**Routine**:
A 16-beat choreography — exactly one Grid — containing four resolved single-bar Waveforms composed with `Routine.Of(w1, w2, w3, w4)`. Callers obtain each Waveform through the ordinary `Random`, `ByName`, or inline `Parse` paths, then compose the value directly; Routine retains no acquisition settings or replacement policy. Its one live read is the current Grid-position envelope (the Grid's bar picks the Waveform), null when there is no Grid; Grid State remains trust data for the consumer. A consumer wanting different values composes another Routine whenever it chooses.
