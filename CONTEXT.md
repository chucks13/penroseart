...
2. **Timing and sequencing**: Each frame applies live Rave OSC to `BeatManager`, updates `BeatManager`, then ticks the Director and then the Switcher, so a handover always precedes execution in the same frame. The Director chooses Standalone, Synced, or Hold behavior; in Synced Mode it rebuilds any player's Cue Sheet whose Structure Generation changed and Casts the on-air player's sheet to the Switcher. The Switcher performs that sheet against the live beat and checks each Cue Mark off as it fires, asking the Director whenever performing needs a decision: what a due mark should actually play, or what to do when the plan cannot feed the playhead — declinable until the wall has held still for four Grid Boundaries, where a fresh card becomes certain.
...
**Tuning Window**:
The canonical Unity workspace for detailed wall observation and Transition tuning. Its focused Live, Rhythm, and Transitions tabs show live sequencing state, musical state, and saved Transition Settings (the retired loaded-cue timeline awaits a track-sheet replacement); the compact Controller Inspector opens it directly. It may steer the Director's Next Transition without taking timing ownership away from the Director. Effect authoring has no dedicated window surface yet.
...
**Director**:
The decision layer that owns *what* plays on the wall. Its whole job in Synced Mode: keep one track-scoped Cue Sheet per player (rebuilt when that player's Structure Generation changes), Cast the on-air player's sheet to the Switcher when the focus changes, and answer the Switcher's two questions: what a planned Cue Mark plays, and what to do at a moment the plan cannot feed — a fresh card or a ride-through. It answers from the sheet and the overrides in force and remembers nothing between asks; every count an answer needs travels with the question. It reads musical truth only from BeatManager and times nothing.
...
_Avoid_: the retired per-Phrase empty-marks sheet (marks now carry assignments); treating it as a queue of pending cues; rebuilding, reordering, or re-dealing it for an override or an Off-Plan Cue (both leave the plan itself untouched); caching or keying sheets by track id.

**Cue Mark**:
A beat position in a Cue Sheet where a stage-directed Cue musically lands, carrying its baked Effect and Transition assignment and the flag recording that it has fired. Marks sit on Grid Boundaries, plus the final beat of a Phrase that is not a whole number of Grids. Consecutive marks are never more than four Grids — 64 beats — apart; that is a ceiling, not a floor, so a short Phrase can place its end mark closer, and a Phrase shorter than one Grid gets an unavoidably short run-in. Phrase boundaries are preferred positions, not mandates. Where a Drop or Fill owns a boundary, the Anchor decides how the moment is performed.
_Avoid_: calling a Cue Mark an Impact Point, Transition start, Transition Completion, or Selected Grid Boundary; empty marks awaiting cast-time selection (retired); reading the four-Grid spacing as a floor as well as a ceiling, or expecting every mark to sit a whole Grid from its neighbour.

**Cast**:
The Director's act of handing the Switcher the Cue Sheet now in force — the cast list for the on-air track, handed over once when the focus changes. An Off-Plan Cue is not a Cast: the Switcher already holds the plan, and the Director simply answers with one freshly dealt card, seeded so no two asks are answered alike.
...
_Avoid_: cutting to a new Effect on the landing beat and calling it a ride-through; suppressing marks at boundaries no Anchor owns.
...
**Bag**:
The seeded fairness mechanism sheet building deals from: one shuffled bag per catalog (Effects, Transitions), dealt top-card so the full catalog shows before any repeat, reshuffled when empty. The reshuffle keeps the card just dealt off the top of the new permutation, because the seam between two passes is the one place a fair bag would otherwise deal the same card twice running — and an Effect dealt twice running bakes a Transition from a card to itself, which moves nothing. Effects are dealt top-card at every mark; only a ride-through carrier digs, because it has to be capable of playing the moment itself. A discard-pile encore is allowed only when an Anchor finds no capable card left, and is the one path that can still repeat a card back to back.
_Avoid_: weights or scoring (the bag *is* the fairness); filtering the deal by energy affinity (removed twice now — energy is a Performer input, not a casting one, per ADR-0011); per-cast random picks; sharing one bag across sheets or players; treating a one-card catalog's unavoidable repeat as a defect.
...
**Off-Plan Cue**:
The cue the Director deals when the plan in force cannot feed the playhead. The Switcher asks for one in exactly two situations: when it re-crosses the Runway beat of a mark that has already fired, and at the Grid Boundary that spends the fourth Grid since the wall last changed. The answer is a fresh card or a ride-through, rolled from the sheet's own stream — a decline costs nothing below the ceiling, where the deal becomes certain, which is what makes the wall holding still past four Grids impossible. It never deals the Effect already on the wall or being moved toward, since a card the wall already holds would answer nothing. Being a pure function of (Structure Generation, player number, boundary beat, ask sequence), it is reproducible without being constant: a loop is dealt something new every pass.
_Avoid_: "staleness cue" (the earlier name, retired with the deficit count it was named after); calling it a Cast, an override, or a Missed Cue's replacement; letting it check a mark off or edit the sheet; asking below the ceiling anywhere but a re-crossed mark, which would pre-empt a plan the playhead is still walking through.

**Missed Cue**:
A Cue Mark the playhead went past without firing, because its Runway beat elapsed while the wall was somewhere else — a fresh Cast, a mid-track focus handover, a needle-drop, a late entry, or an inspection freeze. A cue *is* its Runway, Impact Point, and Tail, so one whose Runway is already behind the playhead cannot be performed as written and is therefore not performed at all; that is what makes all five uneventful. Nothing is checked off, because only firing spends a mark: a passed mark is still plan, and fires as written if the playhead ever reaches its Runway beat again. Until then the four-Grid ceiling is what keeps the wall moving.
_Avoid_: performing a missed mark on the current beat (that was the hard cut this rule removed); compressing the Runway to catch up; treating a passed mark as spent, or as a debt the next Off-Plan Cue pays off.

**Loaded Cue / Lock Point / Standby Cue**:
Retired vocabulary for holding a cue between deciding it and performing it — first cues that had to be loaded, locked, and possibly revoked before they could execute, then one decided cue held against the plan until its Runway beat arrived. The Switcher holds neither. It holds the Cue Sheet and, on each new beat, asks about the mark whose Runway begins there and performs the answer the same frame: a plan plus a beat to recognise, not a lifecycle and not a queue.
_Avoid_: reintroducing commitment verdicts, lock latching, or a revocation window; keeping a decided cue anywhere across beats; reading the Switcher's held sheet or its check-offs as a cue-lifecycle surface.
...
**Show Now**:
The other override, and the opposite of the staged kind: a person picks an Effect and the wall moves to it at once — a keyboard jump, an OSC button, or engaging a Held Effect. It is a performed move, not a cut: the staged Transition runs, starting at that instant with no Runway, because an off-grid interjection has no Impact Point to fly toward. Fire-and-forget — the plan in force is untouched and simply resumes at its next unfired Cue Mark.
_Avoid_: cutting instantly to the Effect (retired); clearing check-offs, the in-force sheet, or any plan state so the wall snaps back off the pick; confusing it with Next Effect, which changes what the *next Cue Mark* moves toward rather than moving the wall now; expecting the pick to survive the next Cue Mark (that is a Held Effect).
...
**Loop**:
A live repeated section of the current music, surfaced as `BeatManager.Loop`. Loops are powers of four and usually preserve Grid, but they rewind or repeat beat numbers, so absolute beat progress goes stale and the same Cue Mark comes around again. Nothing special happens when it does: a performed Cue Mark is checked off permanently, so a loop, a back-cue, and a needle-drop are all just already-performed plan. Sheets are track-scoped, so a loop never re-rolls one.
_Avoid_: assuming a Loop means the wall is out of phase; assuming old absolute progress remains valid after a loop rewind; inferring a loop from beat movement when the lane states it; modeling a loop as its own scheduler or a Director cursor; making loop state the authority on what backward motion means (retired — backward motion means nothing to the Switcher).
...
The mechanism that executes the Cue Sheet it has been handed, unconditionally. It owns all transition timing — Runway, Impact Point, Tail, start, progress, and completion — firing each Cue Mark so the Impact Point lands on it, and checking that mark off as it fires. A check-off is permanent for the life of the handover: the Switcher switches from what is on the wall to what comes next, and never restores what a beat position implies should be there, so re-crossing a fired mark asks for an Off-Plan Cue rather than replaying it. Beside the plan it counts two things, both restarted by a handover: the beat it last acted on, and the Grid Boundaries crossed since the wall last changed — the second is what holds the four-Grid ceiling when the plan has nothing left to give. It chooses no Cue Mark, Performer, or Transition.
_Avoid_: putting musical or casting decisions in the Switcher; reading Phrase, Track Phase, Drop, Fill, or Energy — casting material, as opposed to the beat and Grid lanes it legitimately reads; measuring elapsed music in beats rather than boundary crossings (a loop re-crosses the same beat numbers); re-deciding a Transition mid-flight from the clock; using Switcher progress, completion, or busy state as a scheduling input; treating the plan as a statement of what must be on the wall at a given beat (it says what to perform *at* a beat, once); the retired loaded-cue protocol of verdicts, locks, and revocation (holding a plan and its check-offs is not a lifecycle); calling it "dumb" instead of describing it as execution-only.

**Hold**:
An inspection freeze that suspends the Director so a developer can sit on one effect, watch it, and tweak its settings live — a Unity Editor development affordance, not normal show operation. It is not a selection input and not a second decider: while held, the Director answers that the wall is frozen, so nothing is performed (no rotation, no Cues, no transitions) and the chosen Performer simply stays on screen. Releasing it resumes directing. A Cue Mark whose Runway beat elapsed during the freeze is a Missed Cue: nothing checked it off, but its Runway is behind the playhead, so it does not perform on release — the wall moves again within the four-Grid ceiling instead.
_Avoid_: modeling Hold as a Director selection decision, or as a path that commands the Switcher around the Director; expecting a freeze to defer the cues it covered (they are missed, not queued); re-asserting the held effect every frame (nothing fights it once the one decider is suspended).
...
**Grid Boundary**:
The beat where a Grid starts or ends. A phrase boundary always *begins* a Grid, so the final beat of a Phrase is always eligible as its mandatory final Cue Mark — but it only *completes* one when the Phrase is a whole number of Grids. Grids therefore do not tile a Phrase that is not.
_Avoid_: calling every bar downbeat a Grid Boundary; a Grid Boundary is the 16-beat one, not every 4-beat bar one; assuming a Phrase divides evenly into Grids, or that consecutive Grid Boundaries are always 16 beats apart.
