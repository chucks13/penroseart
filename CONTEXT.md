...
_Avoid_: "Waveform Pattern" (retired name); any Routine length other than one Grid; using "pattern" for one bar's hump sequence; treating a Routine as a new evaluation model; putting re-roll, refresh, wrap, or lifecycle policy on Routine, WaveformSynth, or a base hook; gating evaluation on Grid State.
