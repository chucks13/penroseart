...
public static class EffectFactory {
  ...
  private static string[] GetEffectNames() { return effectNames ?? (effectNames = EffectTypes.Select(t => t.ToString()).ToArray()); }
  ...
  private static Type[] GetEffectTypes() {
    return effectTypes ?? (effectTypes = Assembly.GetAssembly(typeof(EffectBase)).GetTypes().Where(
             myType => myType.IsClass && !myType.IsAbstract &&
                       ...
                       !myType.IsSubclassOf(typeof(Transition))
           ).ToArray());
  }

  public static EffectBase CreateEffect(Type t) { return Activator.CreateInstance(t) as EffectBase; }
