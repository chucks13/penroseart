...
- `SyncedCueIntent.Evaluate(...)` currently protects Drop-aware casting, cadence blocking, manual/held staged choices, and no-deck-mutation when cadence blocks. If Issue 07 evolves this into a pre-lock cue-direction path, preserve those protections in the existing cue/casting seam instead of recreating them in Director or Switcher code.
...
- Preserve the existing `SyncedCueIntent`/`EffectDeckSelection` ownership of preferred Performer casting. Pre-lock polling must be idempotent or mutate the deck only when an offered cue is accepted/committed.
...
- Director/Switcher seam before lock: mutable cue direction updates when `TimingFrame`/casting inputs change, including Phrase length and Drop/manual inputs.
...
- Cue/casting seam: Drop-aligned preferred casting, no preferred Performer, manual staged preservation, cadence-blocked no deck mutation, and repeated pre-lock evaluation without accidental deck rotation.
