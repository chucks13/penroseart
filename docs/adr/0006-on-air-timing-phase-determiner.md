...
## Phrase wins the grid, gated by bar-alignment; boundaries are detected by phrase-start advance

On-Air Timing latches `offset` from the Phrase boundary (the Phrase start *is* a one), falling back to the running `beat`, else assuming 0, then dead-reckons off the 4-count until the next trigger (track-title change, or a Phrase boundary). A re-latch is accepted only when the 4-count is continuous **and** the offset shift is a whole bar; a sub-bar jump is treated as noise (hold, flag CONTRADICTED). This makes lead-ins and non-power-of-two Phrases the same mechanism — re-latch every boundary, flag a Phrase that ends off the 16-grid as irregular — with no special case. A Phrase boundary is detected when the derived `phraseStart = beat − (lengthBeats − beatsUntilNext)` advances, not by label change (adjacent Phrases share labels) nor by a countdown reaching zero (fragile across dropouts). When Phrase look-ahead is present (`beatsUntilNext` + the upcoming Phrase's `lengthBeats`), the next re-latch is predicted then confirmed; when absent, it reacts at the boundary.
...
- **Hard-reset Phase on every track change** — rejected because DJ transitions are beatmatched and usually Phrase-aligned, so the grid typically continues; a hard reset would drop the one at every transition. We soft-hold and force a re-latch at the next Phrase boundary.
