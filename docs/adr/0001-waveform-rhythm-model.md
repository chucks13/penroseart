...
This qualifies as an ADR because it is hard to reverse (effect code and a `StreamingAssets` data file come to depend on the notation), surprising without context (the notation choices below look arbitrary on their face), and the result of real trade-offs.
