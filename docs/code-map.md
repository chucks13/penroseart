...
| `Assets/core/Switching/Switcher.cs` | Holds the handed-over Cue Sheet and its permanent check-offs; thinks once per Grid at Grid start from the on-air beat and Grid, decides the next boundary's mark there (consuming any staged override) and fires it as a mechanical scheduled act at boundary-minus-Runway, owns Runway/Impact/Tail timing and the wall's Grid-counted Stillness (surviving handovers), reports every anomaly — a re-crossed fired mark, a mark blending into the on-wall Effect, Stillness up — through the Director's one `DecideOffPlanCue` doorway (a taken cue performs at the Grid start), and executes StartTransition/RenderAtTime with no cut path and no loaded-cue or lock lifecycle. |
