/// <summary>
/// The 16-beat Grid arithmetic, in one place. Grid is exact integer modular math: the
...
/// offset. <see cref="GridSync"/> (stateful determiner) uses this, so the grid math is defined
/// exactly once.
/// </summary>
public static class Grid
{
    /// <summary>Beats in one Grid cycle — the 16-beat grid the one repeats on.</summary>
    ...
    public static int OffsetForPhraseStart(int phraseStartBeat) => Mod(phraseStartBeat - 1, GridBeats);
    ...
    public static int PositionFor(int beat, int offset) => Mod((beat - 1) - offset, GridBeats) + 1;
    ...
    /// grids — its length is not a multiple of <see cref="GridBeats"/>. The single definition of
    /// phrase irregularity, shared by GridSync (which drives CONTRADICTED) and PhraseTracker (the
    /// Director's diagnostic), so the two never silently diverge. Callers guard non-positive lengths.
    /// </summary>
    public static bool IsIrregularPhrase(int phraseLengthBeats) => phraseLengthBeats % GridBeats != 0;
}
