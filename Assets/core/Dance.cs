...
public class Dance {
  ...
  public void MarkBeat() { trigger = true; }

  // Update is called once per frame
  public void Update() {
    // simple interface, when is the beat, and how long since last beat.
    float now   = Time.time;      // FixedTime is for FixedUpdate 
    float delta = Time.deltaTime; // fixedDeltaTime is for FixedUpdate
    ...
    if(Mathf.Abs(duration) > 0.0001f) // (duration != 0f) <== a float is never truly zero 
    {
