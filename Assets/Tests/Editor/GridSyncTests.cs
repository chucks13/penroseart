...
/// <summary>
/// Encodes the locked Grid contract (ADR-0006 + on-air-timing-redesign-2026-06-24.html) as
/// scripted DJ timelines driven through <see cref="GridTimelineHarness"/>. Slice 01 is the
/// TDD red phase: <see cref="GridSync"/> is a skeleton, so these assert the externally visible
/// <see cref="GridReading"/> contract and run red until slice 02 lands the held-offset model.
///
/// Grid math under test: offset = (phraseStart − 1) mod 16, and
/// position = ((beat − 1) − offset) mod 16 + 1. A frame on a Phrase start always reads position 1.
/// </summary>
public sealed class GridSyncTests
{
    [Test]
    public void SteadyAdvance_HoldsOffsetAndAdvancesPosition()
    {
        var readings = GridTimelineHarness.Run(
        ...
        Assert.That(now.State, Is.EqualTo(GridSyncState.Locked));
    ...
    public void OneBarLoop_StepsPositionBackFourWithMusic()
    {
        ...
        Assert.That(afterLoop.State, Is.EqualTo(GridSyncState.Locked));
    }
    ...
    [Test]
    public void FourBarLoop_LeavesGridUndisturbed()
    {
        var readings = GridTimelineHarness.Run(
        ...
        Assert.That(afterLoop.Offset, Is.EqualTo(0));
        Assert.That(afterLoop.Position, Is.EqualTo(1), "A 16-beat loop is on the grid: the position is identical.");
        Assert.That(afterLoop.State, Is.EqualTo(GridSyncState.Locked));
        Assert.That(afterLoop.IsContradicted, Is.False);
    }

    [Test]
    public void EightBarLoop_LeavesGridUndisturbed()
    {
        var readings = GridTimelineHarness.Run(
        ...
        var afterLoop = readings[1];
        Assert.That(afterLoop.Offset, Is.EqualTo(0));
        Assert.That(afterLoop.Position, Is.EqualTo(16), "A 32-beat loop is on the grid: the position is identical.");
        Assert.That(afterLoop.State, Is.EqualTo(GridSyncState.Locked));
    }

    [Test]
    public void SixteenBarLoop_LeavesGridUndisturbed()
    {
        var readings = GridTimelineHarness.Run(
        ...
        var afterLoop = readings[1];
        Assert.That(afterLoop.Offset, Is.EqualTo(0));
        Assert.That(afterLoop.Position, Is.EqualTo(8), "A 64-beat loop is on the grid: the position is identical.");
        Assert.That(afterLoop.State, Is.EqualTo(GridSyncState.Locked));
    }
    ...
    public void LeadIn_LandsPositionOneViaTheSameOffsetMechanism()
    {
        ...
        Assert.That(atFirstOne.State, Is.EqualTo(GridSyncState.Locked));
    }

    [Test]
    public void IrregularPhrase_ReportsContradictedThenReLatchesToLockedAtTheNextRegularBoundary()
    {
        // A 24-beat phrase is not a multiple of 16, so it cannot subdivide into whole 16-beat grids.
        // GridSync reports CONTRADICTED for the phrase's duration while still holding a usable position
        ...
        Assert.That(irregular.State, Is.EqualTo(GridSyncState.Contradicted),
        ...
        Assert.That(reLatched.State, Is.EqualTo(GridSyncState.Locked),
    ...
    [Test]
    public void ForwardSeek_ReEstablishesGridAtTheNextBoundary()
    {
        var readings = GridTimelineHarness.Run(
        ...
        Assert.That(reEstablished.State, Is.EqualTo(GridSyncState.Locked));
    }
    ...
    public void PhraseDataDropout_CoastsAndHoldsTheOffset()
    {
        ...
        Assert.That(coasting.State, Is.EqualTo(GridSyncState.Coasting));
    ...
    public void TrackChange_ResetsThenReAcquiresAtTheNewSongsFirstBoundary()
    {
        ...
        // `beat` is a per-track counter, so the old offset is meaningless. GridSync resets and
        ...
        Assert.That(reset.State, Is.EqualTo(GridSyncState.Coasting), "A beat-only guess pending the new song's first Phrase is COASTING, not a lock.");
        ...
        Assert.That(reAcquired.State, Is.EqualTo(GridSyncState.Locked));
    }
    ...
    public void TransientTitleBlank_KeepsTheHeldOffsetWhenTheSameTrackResumes()
    {
        ...
        Assert.That(resumed.State, Is.EqualTo(GridSyncState.Coasting), "Phrase feed is still out, so it coasts on the held offset.");
    }
    ...
    public void ClockLoss_SignalsStandAloneFloor()
    {
        ...
        Assert.That(floor.Position, Is.EqualTo(-1), "There is no Grid position without a clock.");
    }
    ...
    public void NoPhrase_FallsBackToTheRunningBeatGrid()
    {
        ...
        // never sends Phrase data): GridSync lines the grid up on the running beat itself — offset 0,
        ...
        Assert.That(fallback.State, Is.EqualTo(GridSyncState.Coasting), "A beat-only guess is not a phrase lock.");
    ...
    public void Contradiction_ClearsToCoastingWhenThePulseRecoversDuringADropout()
    {
        ...
        Assert.That(recovered.State, Is.EqualTo(GridSyncState.Coasting), "With the Phrase feed gone it coasts on the held offset.");
